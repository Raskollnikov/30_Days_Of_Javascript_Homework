...
write to show output on the browser document(view port). Both methods used only for testing 
and debugging purposes. The console method is the most popular testing and debugging tool on the browser.
We use document.getElementById() when we like to interact with DOM try using JavaScript. 
We will cover DOM in another section.

In addition to the famous, console.log() method, the console provides other 
more methods.


console.log()
We use console.log() to show output on the browser console. We can substitute values and also we
can style the logging out put using %c.
...
console.warn()
We use console.warn() to give warning on browser. For instance to inform or warn deprecation of version of a package or bad practices. Copy 
the following code and paste it on browser console to see warning messages.
...
const counties=['finland','denmark','german','italy','france','england','georgia','armenia','USA','Canada'];
...
// 1.1) Display the countries array as a table
console.table(counties);
// 1.2) Display the countries object as a table
console.table(countryListAlpha3);
// 1.3) Use console.group() to group logs
// console.group('this is countries');
// console.log(counties)
// console.groupEnd();
// 1.4) 10 > 2 * 10 use console.assert()
// console.assert(20>10)
// 1.5) Write a warning message using console.warn()
// console.warn('Restart Your Pc!');
// 1.6) Write an error message using console.error()
// console.error('this is 404 Error!')